...
import { themeColumnList } from '../../utils/table/tableColumn';
import { GET_THEMES_BY_PAGING } from '../../utils/api/themeApi';
import ThemeModalData from './ThemeModalData';
...
import { usePageContainerContext } from '../layout/PageContainer';
...
const ThemeList = () => {
  ...
  // query the theme list
  // without filter
  const [loading, tableData, data] = useQueryTableData(GET_THEMES_BY_PAGING, 'themesByPaging', {
    pageNo: page
  })

  // filter theme by category
  // query the filtered theme
  const { filteredData } = usePageContainerContext()
  ...
  let image = ''
  if (modalData.icon) {
    image = modalData.icon.path
  }

  // pagination
  const pageLength = data?.themesByPaging.totalPages
  const currentPage = data?.themesByPaging.currentPage
  ...
  return (
    <>
      <TableContainer
        ...
        modalComponent={ThemeModalData}
        image={image}
        onEditClick={`/translation_topics/themes/edit_theme/${modalData.id}`}
      />
